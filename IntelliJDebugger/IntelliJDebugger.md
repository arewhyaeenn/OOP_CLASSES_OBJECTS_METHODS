...
In a project consisting of the two classes provided above, set a **break point** on the line with the statement `double tempInCelcius = 100;`. You can set a break point on any line by clicking in between the editor pane's line numbers and the text editor itself. A red dot will appear on the line, denoting that there is a break point there:
...
Finally, note that an extra value can be seen in the call stack. Before we only had "main:7, Client" (indicating a statement on line 7 of the `main` function in the `Client.java`). Now we have 2 elements. The new element (on top) is in the `celcToFahren` function; more specifically it is on line 10 of `TemperatureConversion.java`. This indicates that the `TemperatureConversion.celcToFahren` call originaged from (or "is part of the execution of") the statement in `Client.java`'s `main` function, on line 7.
...
Many programmers are tempted to debug their software by printing status messages to the console stating the values of variables and other pertinent information. This is generally bad practice except in niche cases where the debugger isn't helpful for one reason or another or isn't available. It's bad practice for a few reasons, but the primary one is that it is **tedious** and **slow** relative to using the debugger, which is **easy** and **fast** once you're used to it! 
